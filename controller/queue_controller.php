...
class queue_controller
{
    ...
    public function display()
    {
        ...
        $queue_operational_focus = $this->build_queue_operational_focus($filtered_rows);
        ...
        foreach ($this->safe_bucket($queue_operational_focus, 'cards') as $operations_card)
        {
            $this->template->assign_block_vars('helpdesk_queue_operation_cards', $operations_card);
        }
        ...
        $this->template->assign_vars([
            ...
            'HELPDESK_QUEUE_OPERATIONS_STATUS_LABEL' => $queue_operational_focus['status_label'],
            'HELPDESK_QUEUE_OPERATIONS_STATUS_TEXT' => $queue_operational_focus['status_text'],
            'HELPDESK_QUEUE_OPERATIONS_STATUS_CLASS' => $queue_operational_focus['status_class'],
            'HELPDESK_QUEUE_OPERATIONS_AUTOMATION_TEXT' => $queue_operational_focus['automation_text'],
            'HELPDESK_QUEUE_OPERATIONS_NOTIFICATIONS_TEXT' => $queue_operational_focus['notifications_text'],
    ...
    public function my_tickets()
    {
        ...
        $feedback_map = $this->feedback_enabled() ? $this->feedback_map_for_topics(array_map(function ($row) {
            return (int) ($row['TOPIC_ID'] ?? 0);
        }, $my_rows)) : [];

        $counts = $this->build_my_ticket_counts($my_rows);
        $feedback_counts = $this->build_my_ticket_feedback_counts($my_rows, $feedback_map);
        $due_today_count = $this->count_my_ticket_flag($my_rows, 'IS_DUE_TODAY');
        $needs_reply_count = $this->count_my_ticket_flag($my_rows, 'IS_STAFF_REPLY');
        ...
        foreach ($paged_rows as $row)
        {
            $feedback_state = $this->my_ticket_feedback_state($row, $feedback_map);
            $this->template->assign_block_vars('helpdesk_my_ticket_rows', $row + [
                'META_LINE' => $this->build_my_ticket_meta_line($row),
                'S_FEEDBACK_STATE' => $feedback_state['label'] !== '',
                'FEEDBACK_STATE_LABEL' => $feedback_state['label'],
                'FEEDBACK_STATE_CLASS' => $feedback_state['class'],
                'FEEDBACK_STATE_NOTE' => $feedback_state['note'],
                'ACTION_TEXT' => $this->build_my_ticket_action_text($row),
            ]);
        }
        ...
        $this->template->assign_vars([
            ...
            'HELPDESK_MY_TICKETS_FEEDBACK_PENDING_COUNT' => $feedback_counts['pending'],
            'HELPDESK_MY_TICKETS_FEEDBACK_SUBMITTED_COUNT' => $feedback_counts['submitted'],
            'HELPDESK_MY_TICKETS_AUTOMATION_NOTE' => $this->my_tickets_automation_note($due_today_count, $needs_reply_count),
            'HELPDESK_MY_TICKETS_NOTIFICATION_NOTE' => $this->my_tickets_notification_note(),
    ...
    protected function feedback_map_for_topics(array $topic_ids)
    {
        $topic_ids = array_values(array_unique(array_filter(array_map('intval', $topic_ids))));
        if (empty($topic_ids) || !$this->feedback_enabled())
        {
            return [];
        }

        $sql = 'SELECT topic_id, rating, submitted_time
    ...
    protected function build_my_ticket_feedback_counts(array $rows, array $feedback_map)
    {
        $counts = ['pending' => 0, 'submitted' => 0];
        foreach ($rows as $row)
        {
            $state = $this->my_ticket_feedback_state($row, $feedback_map);
            if ($state['key'] === 'pending')
            {
                $counts['pending']++;
            }
            else if ($state['key'] === 'submitted')
            {
                $counts['submitted']++;
            }
        }
    ...
    protected function my_ticket_feedback_state(array $row, array $feedback_map)
    {
        $topic_id = (int) ($row['TOPIC_ID'] ?? 0);
        $status_tone = (string) ($row['STATUS_TONE'] ?? '');
        $has_feedback = $topic_id > 0 && isset($feedback_map[$topic_id]);

        if ($has_feedback)
        {
            return [
                'key' => 'submitted',
                'label' => $this->user->lang('HELPDESK_MY_TICKETS_FEEDBACK_SUBMITTED'),
                'class' => 'helpdesk-feedback-state-submitted',
                'note' => $this->user->lang('HELPDESK_MY_TICKETS_FEEDBACK_SENT'),
            ];
        }

        if (in_array($status_tone, ['resolved', 'closed'], true))
        {
            return [
                'key' => 'pending',
                'label' => $this->user->lang('HELPDESK_MY_TICKETS_FEEDBACK_PENDING'),
                'class' => 'helpdesk-feedback-state-pending',
                'note' => $this->user->lang('HELPDESK_MY_TICKETS_FEEDBACK_AWAITING'),
            ];
        }

        return ['key' => 'none', 'label' => '', 'class' => '', 'note' => ''];
    }
    ...
    protected function load_ticket_rows(array $forum_ids, $apply_default_sort = true)
    {
        ...
        foreach ($rows as $index => $queue_row)
        {
            $next_action = $this->build_queue_row_next_action($queue_row);
            $rows[$index]['NEXT_ACTION_LABEL'] = $next_action['label'];
            $rows[$index]['NEXT_ACTION_TEXT'] = $next_action['text'];
            $rows[$index]['NEXT_ACTION_CLASS'] = $next_action['class'];
            $rows[$index]['AUTOMATION_NOTE'] = $this->build_queue_row_automation_note($rows[$index]);
        }
...
\redirect($this->queue_redirect_url($updated_count > 0 ? 'redistributed_department' : 'noop'));
    ...
    protected function department_profile_reply_template_label($department_key)
    {
        ...
        return !empty($definitions[$department_key]['reply_template']) ? trim((string) $definitions[$department_key]['reply_template']) : '';
    }
...
protected function smart_redistribution_reason(array $ticket_row, array $target)
{
    ...
    protected function visible_helpdesk_forum_ids()
    {
        ...
        {
            if ($this->can_view_helpdesk_forum($forum_id))
            {
    ...
    protected function build_my_ticket_action_text(array $row)
    {
        if (empty($row['IS_OPEN']))
        {
            return $this->user->lang('HELPDESK_MY_TICKETS_ACTION_COMPLETED');
        }

        if (isset($row['REPLY_COUNT']) && (int) $row['REPLY_COUNT'] <= 0)
        {
            return $this->user->lang('HELPDESK_MY_TICKETS_ACTION_FIRST_REPLY');
        }

        if (!empty($row['IS_STAFF_REPLY']))
        {
            return $this->user->lang('HELPDESK_MY_TICKETS_ACTION_WAITING_TEAM');
        }

        if (!empty($row['IS_UNASSIGNED']))
        {
            return $this->user->lang('HELPDESK_MY_TICKETS_ACTION_ASSIGNMENT');
        }

        if (!empty($row['IS_OVERDUE']))
        {
            return $this->user->lang('HELPDESK_MY_TICKETS_ACTION_PRIORITY');
        }

        if (!empty($row['IS_STALE']))
        {
            return $this->user->lang('HELPDESK_MY_TICKETS_ACTION_RESUME');
        }

        if (!empty($row['IS_DUE_TODAY']))
        {
            return $this->user->lang('HELPDESK_MY_TICKETS_ACTION_DUE_TODAY');
        }

        return $this->user->lang('HELPDESK_MY_TICKETS_ACTION_MONITOR');
    }

    protected function count_my_ticket_flag(array $rows, $flag_key)
    {
        $count = 0;
        foreach ($rows as $row)
        {
    ...
    protected function my_tickets_automation_note($due_today_count, $needs_reply_count)
    {
        return $this->user->lang('HELPDESK_MY_TICKETS_AUTOMATION_NOTE', (int) $due_today_count, (int) $needs_reply_count);
    }

    protected function my_tickets_notification_note()
    {
        if (!$this->email_notifications_enabled())
        {
            return $this->user->lang('HELPDESK_MY_TICKETS_NOTIFICATION_NOTE_DISABLED');
        }

        if ($this->email_notify_author_enabled())
        {
            return $this->user->lang('HELPDESK_MY_TICKETS_NOTIFICATION_NOTE_ENABLED');
        }

        return $this->user->lang('HELPDESK_MY_TICKETS_NOTIFICATION_NOTE_PARTIAL');
    }

    protected function build_queue_operational_focus(array $rows)
    {
        $immediate_count = 0;
        $next_count = 0;
        $rebalance_count = 0;
        ...
        foreach ($rows as $row)
        {
            ...
            if (!empty($row['IS_OVERDUE']) || !empty($row['IS_CRITICAL']))
            {
                $immediate_count++;
            }
            if (!empty($row['IS_OVERDUE']))
            ...
            if ((isset($row['REPLY_COUNT']) && (int) $row['REPLY_COUNT'] <= 0) || !empty($row['IS_STAFF_REPLY']) || !empty($row['IS_DUE_TODAY']))
            {
                $next_count++;
            }
            if (isset($row['REPLY_COUNT']) && (int) $row['REPLY_COUNT'] <= 0)
            ...
            if (!empty($row['IS_UNASSIGNED']) || !empty($row['IS_REDISTRIBUTION_CANDIDATE']) || !empty($row['IS_WORKLOAD_OVERLOADED']))
            {
                $rebalance_count++;
            }
            if (!empty($row['IS_UNASSIGNED']))
            ...
            if (!empty($row['IS_REDISTRIBUTION_CANDIDATE']) || !empty($row['IS_WORKLOAD_OVERLOADED']))
            {
                $has_redistribution = true;
            }

            if (!empty($row['S_HAS_REPLY_TEMPLATES']))
            {
                $template_count++;
            }
        }
        ...
        {
            $status_label = $this->user->lang('HELPDESK_TEAM_QUEUE_OPERATIONS_STATUS_CRITICAL');
            $status_class = 'critical';
            $status_text = $this->user->lang('HELPDESK_TEAM_QUEUE_OPERATIONS_STATUS_TEXT_CRITICAL');
        }
        else if ($next_count > 0 || $rebalance_count > 0)
        {
            $status_label = $this->user->lang('HELPDESK_TEAM_QUEUE_OPERATIONS_STATUS_ATTENTION');
            $status_class = 'attention';
            $status_text = $this->user->lang('HELPDESK_TEAM_QUEUE_OPERATIONS_STATUS_TEXT_ATTENTION');
        }
        else
        {
            $status_label = $this->user->lang('HELPDESK_TEAM_QUEUE_OPERATIONS_STATUS_STABLE');
            $status_class = 'stable';
            $status_text = $this->user->lang('HELPDESK_TEAM_QUEUE_OPERATIONS_STATUS_TEXT_STABLE');
        }
        ...
        if ($this->safe_count($this->priority_queue_definitions()) > 0)
        {
            $automation_score++;
        }
        if ($this->assignment_enabled())
        ...
        if ($this->safe_count($this->assignee_queue_definitions()) > 0)
        {
            $automation_score++;
        }
        ...
        {
            $automation_text = $this->user->lang('HELPDESK_TEAM_QUEUE_OPERATIONS_AUTOMATION_TEXT_ENABLED');
        }
        else if ($automation_score > 0)
        {
            $automation_text = $this->user->lang('HELPDESK_TEAM_QUEUE_OPERATIONS_AUTOMATION_TEXT_PARTIAL');
        }
        else
        {
            $automation_text = $this->user->lang('HELPDESK_TEAM_QUEUE_OPERATIONS_AUTOMATION_TEXT_MINIMAL');
        }

        if (!$this->email_notifications_enabled())
        {
            $notifications_text = $this->user->lang('HELPDESK_TEAM_QUEUE_OPERATIONS_NOTIFICATIONS_TEXT_DISABLED');
        }
        else if ($this->email_notify_author_enabled() && $this->email_notify_user_reply_enabled())
        {
            $notifications_text = $this->user->lang('HELPDESK_TEAM_QUEUE_OPERATIONS_NOTIFICATIONS_TEXT_ENABLED');
        }
        else
        {
            $notifications_text = $this->user->lang('HELPDESK_TEAM_QUEUE_OPERATIONS_NOTIFICATIONS_TEXT_PARTIAL');
        }

        $cards = [
            [
                'COUNT' => $immediate_count,
                'TITLE' => $this->user->lang('HELPDESK_TEAM_QUEUE_OPERATIONS_CARD_IMMEDIATE_TITLE'),
                'META' => $this->user->lang('HELPDESK_TEAM_QUEUE_OPERATIONS_CARD_IMMEDIATE_META'),
                'U_ROW' => $this->queue_scope_url($has_overdue ? 'overdue' : 'critical'),
                'STATUS_CLASS' => 'is-critical',
            ],
            [
                'COUNT' => $next_count,
                'TITLE' => $this->user->lang('HELPDESK_TEAM_QUEUE_OPERATIONS_CARD_NEXT_TITLE'),
                'META' => $this->user->lang('HELPDESK_TEAM_QUEUE_OPERATIONS_CARD_NEXT_META'),
                'U_ROW' => $this->queue_scope_url($has_no_reply ? 'no_reply' : ($has_staff_reply ? 'staff_reply' : ($has_due_today ? 'due_today' : 'all'))),
                'STATUS_CLASS' => 'is-attention',
            ],
            [
                'COUNT' => $rebalance_count,
                'TITLE' => $this->user->lang('HELPDESK_TEAM_QUEUE_OPERATIONS_CARD_REBALANCE_TITLE'),
                'META' => $this->user->lang('HELPDESK_TEAM_QUEUE_OPERATIONS_CARD_REBALANCE_META'),
                'U_ROW' => $this->queue_scope_url($has_unassigned ? 'unassigned' : ($has_redistribution ? 'redistribute' : 'overloaded')),
                'STATUS_CLASS' => 'is-attention',
            ],
            [
                'COUNT' => $template_count,
                'TITLE' => $this->user->lang('HELPDESK_TEAM_QUEUE_OPERATIONS_CARD_TEMPLATES_TITLE'),
                'META' => $this->user->lang('HELPDESK_TEAM_QUEUE_OPERATIONS_CARD_TEMPLATES_META'),
                'U_ROW' => $this->queue_view_url('queue'),
                'STATUS_CLASS' => 'is-stable',
            ],
        ];
    ...
    protected function build_queue_row_next_action(array $row)
    {
        if (empty($row['IS_OPEN']))
        {
            return [
                'label' => $this->user->lang('HELPDESK_TEAM_QUEUE_ROW_NEXT_ACTION_COMPLETED'),
                'text' => $this->user->lang('HELPDESK_TEAM_QUEUE_ROW_NEXT_ACTION_COMPLETED_EXPLAIN'),
                'class' => 'stable',
            ];
        }
        ...
        {
            return [
                'label' => $this->user->lang('HELPDESK_TEAM_QUEUE_ROW_NEXT_ACTION_ESCALATE'),
                'text' => $this->user->lang('HELPDESK_TEAM_QUEUE_ROW_NEXT_ACTION_ESCALATE_EXPLAIN'),
                'class' => 'critical',
            ];
        }
        ...
        {
            return [
                'label' => $this->user->lang('HELPDESK_TEAM_QUEUE_ROW_NEXT_ACTION_FIRST_REPLY'),
                'text' => $this->user->lang('HELPDESK_TEAM_QUEUE_ROW_NEXT_ACTION_FIRST_REPLY_EXPLAIN'),
                'class' => 'attention',
            ];
        }
        ...
        {
            return [
                'label' => $this->user->lang('HELPDESK_TEAM_QUEUE_ROW_NEXT_ACTION_WAITING_TEAM'),
                'text' => $this->user->lang('HELPDESK_TEAM_QUEUE_ROW_NEXT_ACTION_WAITING_TEAM_EXPLAIN'),
                'class' => 'attention',
            ];
        }
        ...
        {
            return [
                'label' => $this->user->lang('HELPDESK_TEAM_QUEUE_ROW_NEXT_ACTION_ASSIGN'),
                'text' => $this->user->lang('HELPDESK_TEAM_QUEUE_ROW_NEXT_ACTION_ASSIGN_EXPLAIN'),
                'class' => 'attention',
            ];
        }
        ...
        {
            return [
                'label' => $this->user->lang('HELPDESK_TEAM_QUEUE_ROW_NEXT_ACTION_TODAY'),
                'text' => $this->user->lang('HELPDESK_TEAM_QUEUE_ROW_NEXT_ACTION_TODAY_EXPLAIN'),
                'class' => 'attention',
            ];
        }

        if (!empty($row['IS_STALE']) || !empty($row['IS_ATTENTION']))
        {
            return [
                'label' => $this->user->lang('HELPDESK_TEAM_QUEUE_ROW_NEXT_ACTION_RESUME'),
                'text' => $this->user->lang('HELPDESK_TEAM_QUEUE_ROW_NEXT_ACTION_RESUME_EXPLAIN'),
                'class' => 'attention',
            ];
        }

        return [
            'label' => $this->user->lang('HELPDESK_TEAM_QUEUE_ROW_NEXT_ACTION_MONITOR'),
            'text' => $this->user->lang('HELPDESK_TEAM_QUEUE_ROW_NEXT_ACTION_MONITOR_EXPLAIN'),
            'class' => 'stable',
        ];
    }

    protected function build_queue_row_automation_note(array $row)
    {
        $notes = [];

        if (!empty($row['IS_OVERDUE']))
        {
            $notes[] = $this->user->lang('HELPDESK_TEAM_QUEUE_ROW_AUTOMATION_SLA_OVERDUE');
        }
        else if (!empty($row['SLA_DEADLINE_AT']))
        {
            $notes[] = $this->user->lang('HELPDESK_TEAM_QUEUE_ROW_AUTOMATION_SLA_DUE', (string) $row['SLA_DEADLINE_AT']);
        }

        if (!empty($row['S_HAS_REPLY_TEMPLATES']))
        {
            $notes[] = $this->user->lang('HELPDESK_TEAM_QUEUE_ROW_AUTOMATION_TEMPLATE_READY');
        }
    ...
    protected function can_view_helpdesk_forum($forum_id)
    {
        $forum_id = (int) $forum_id;
        if ($forum_id <= 0)
        {
        ...
        return $this->auth->acl_get('f_list', $forum_id)
            && $this->auth->acl_get('f_read', $forum_id)
            && $this->auth->acl_get('f_helpdesk_view', $forum_id);
    }
    ...
    protected function priority_queue_definitions()
    {
        return $this->department_priority_queue_definitions();
    }
